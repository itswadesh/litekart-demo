<template>
  <div class="w-full mt-10">
    <div class="right-0 pb-3 mb-6 text-sm leading-relaxed text-gray-500 4/5 bb">
      <div>
        ...
        <div class="flex justify-between mt-1">
          ...
          <div class="ml-2 text-right text-black-400">{{cart.subtotal| currency}}</div>
        </div>
        <div
        ...
        >
          ...
          <div class="ml-2 text-right text-black-400">{{cart.discount | currency}}</div>
        </div>

        <div class="flex justify-between mt-1">
          <div class="text-sm text-left">Order Total</div>
          <div class="ml-2 text-right text-black-400">{{cart.subtotal - cart.discount | currency}}</div>
        </div>
        <div class="flex justify-between mt-1">
    ...
    <div class="fixed bottom-0 left-0 w-full px-4 pb-2 bg-white lg:relative lg:pr-0">
      <div class="flex justify-between mt-2">
        <div class="text-lg font-bold text-left">Total</div>
        <div class="font-bold text-right text-black">{{cart.total | currency}}</div>
      </div>
      <Button
        @click="proceed"
        color="primary"
