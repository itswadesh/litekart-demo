<template>
  <div class="flex justify-between pt-5 pb-5 border-b border-gray-300">
    <div class="lg:w-1/5 left-0 xs:w-1/6">
      <div>
        <img
          class="lg:rounded xs:rounded-b-none w-48"
    ...
    <div class="lg:w-4/5 right-0 xs:9/12">
      <div class="pl-4 font-hairline">
        <p class="text-black mb-2">
          <nuxt-link :to="`/${item.product.slug}?id=${item.product._id}`">{{item.product.name}}</nuxt-link>
        </p>
        <div class="flex">
          ...
          >Hurry! Only {{item.variant.stock}} left in stock</span>
        </div>
        <p class="relative mb-2">
          <span class="text-black font-bold mb-2 text-2xl">{{item.variant.price | currency}}</span>
          <span class="text-gray-500 line-through ml-2 text-xs lg:text-sm">{{item.variant.mrp | currency}}</span>
          <span class="ml-2 text-green-400 text-xs lg:text-sm">{{calculateOffPercent(item.variant.mrp, item.variant.price)}}% off</span>
        </p>
        <div class="flex justify-between flex-wrap text-sm">
          ...
          <div class="w-full lg:w-2/5 flex lg:justify-end align-middle lg:text-sm py-3">
            <button class="mr-6 focus:outline-none">MOVE TO WISHLIST</button>
            <button
              class="text-gray-600 bg-gray-300 px-2 rounded"
